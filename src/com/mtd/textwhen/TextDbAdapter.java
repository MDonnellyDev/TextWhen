...
public class TextDbAdapter {
	...
	public long getEntryRow(OutgoingText text) {
		...
		DatabaseUtils.appendEscapedSQLString(sb, where);

		Cursor cursor = db.query(true, DB_TABLE, new String[] { KEY_ID },
				sb.toString(), null, null, null, null, null);
	...
	public Cursor getAllEntries() {
		return db.query(DB_TABLE, new String[] { KEY_ID, KEY_RECIPIENT,
				...
				KEY_GMT_OFFSET }, null, null, null, null, null);
	}
	...
	public List<OutgoingText> getAllEntriesList() {
		...
		Cursor cursor = db.query(true, DB_TABLE, new String[] { KEY_ID,
				...
				KEY_MODIFIED, KEY_GMT_OFFSET }, null, null, null, null,
				KEY_SCHEDULED, null);
		if (cursor.moveToFirst()) {
			do {
				...
				try {
					textList.add(new OutgoingText(recipient, subject, body,
